//Yes, I completely stole this
//1.  Updated the using for EF6 using System.Data.Entity.Core.Objects
//2.  Added parameter for System.Data.Entity.Core.EntityClient.EntityConnection so Effort framework could be used for in memory Entity Framework tests.
...
namespace PharmacyAdjudicator.Library 
{
    ...
    public class DbContextManager<C> : IDisposable where C : DbContext
    {
        ...
        {
            return GetManager(database, "default", null, null);
        }
        ...
        {
            return GetManager(database, label, null, null);
        }
        ...
        {
            return GetManager(database, "default", model, null);
        }

        public static DbContextManager<C> GetManager(System.Data.Entity.Core.EntityClient.EntityConnection connection)
        {
            return GetManager(string.Empty, "default", null, connection);
        }
        ...
        public static DbContextManager<C> GetManager(string database, string label, DbCompiledModel model, System.Data.Entity.Core.EntityClient.EntityConnection connection)
        {
            lock (_lock)
            {
                ...
                {
                    mgr = new DbContextManager<C>(database, label, model, null, connection);
        ...
        public static DbContextManager<C> GetManager(ObjectContext context, string label)
        {
            lock (_lock)
            {
                ...
                {
                    mgr = new DbContextManager<C>(null, label, null, context, null);
                    mgr.ContextLabel = contextLabel;
                    ApplicationContext.LocalContext[contextLabel] = mgr;
                }
                mgr.AddRef();
        ...
        private DbContextManager(string database, string label, DbCompiledModel model, ObjectContext context, System.Data.Entity.Core.EntityClient.EntityConnection connection)
        {
            _label = label;
            ...
            else if (string.IsNullOrEmpty(database))
            {
                if (connection == null)
                    ...
                    _context = (C)(Activator.CreateInstance(typeof(C), connection));
            }
            else
            {
